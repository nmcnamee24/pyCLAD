"""Clean-room recreation of the COMMAND paper architecture.

This module keeps the architecture described in Liu et al. (2026) separate
from the smaller clean-room baseline in :mod:`architecture`.  Training-budget
choices such as epochs, batch size, and learning-rate schedules deliberately
do not appear here.

The paper completely specifies the module graph but leaves a few internal
dimensions unspecified.  :class:`PaperCommandArchitectureConfig` uses the
published values where available and the authors' public experiment script
only for omitted values.  See ``PAPER_REPRODUCTION.md`` in this directory for
the source of every default and the remaining ambiguities.
"""
...
class PaperCommandArchitectureConfig:
...
class PaperCommandOutput:
    ...
    memory: PaperDualMemoryOutput


class PaperAugFuseNet(nn.Module):
...
class PaperChannelAttention1D(nn.Module):
...
class PaperTemporalAttention1D(nn.Module):
...
class PaperCBAM1D(nn.Module):
    ...
    def __init__(self, channels: int, reduction: int, temporal_kernel_size: int, dropout: float):
        super().__init__()
        self.channel_attention = PaperChannelAttention1D(channels, reduction, dropout)
        self.temporal_attention = PaperTemporalAttention1D(temporal_kernel_size, dropout)
...
class PaperTempMamba(nn.Module):
...
class PaperMemDualNet(nn.Module):
    ...
    def forward(self, features: Tensor, *, track_usage: bool = False) -> PaperDualMemoryOutput:
...
class PaperCommandNetwork(nn.Module):
    """AugFuseNet -> TempMamba+CBAM -> MemDualNet -> scoring heads."""

    def __init__(self, config: PaperCommandArchitectureConfig | None = None):
        super().__init__()
        self.config = config or PaperCommandArchitectureConfig()
        self.feature_fusion = PaperAugFuseNet(self.config.appearance_dim, self.config.motion_dim)
        self.temporal = PaperTempMamba(self.config)
        self.memory = PaperMemDualNet(self.config.fused_dim, self.config.memory_size)
    ...
    def forward(self, appearance: Tensor, motion: Tensor, *, track_memory_usage: bool = False) -> PaperCommandOutput:
