...
class TestCommandUcfCrimeDataset:
    ...
    def setup_method(self):
        ...
        train_anomaly = "Abuse/Abuse001_x264.mp4\nArrest/Arrest001_x264.mp4\n"
        train_normal = "Normal_Videos_event/Normal001_x264.mp4\nNormal_Videos_event/Normal002_x264.mp4\n"
        ...
        (self.root / "test_normalv2.txt").write_text("Normal_Videos_event/Normal003_x264.mp4 64 -1\n", encoding="utf-8")
        paths = (
    ...
    def _dataset(self):
        ...
        return CommandUcfCrimeDataset(self.root, concept_order=("Abuse", "Arrest"))

    def test_balanced_training_concepts_pack_weak_targets(self):
        dataset = self._dataset()
        concepts = dataset.training_concepts()
        assert [concept.name for concept in concepts] == ["Abuse", "Arrest"]
        for concept in concepts:
            assert concept.features.shape == (4, 2048)
            assert concept.data.shape == (4, 2050)
            assert {window.payload["record_index"] for window in concept.windows} == {0, 1}
            np.testing.assert_array_equal(np.sort(np.unique(concept.strategy_targets["weak_label"])), [0.0, 1.0])
            assert len(np.unique(concept.strategy_targets["bag_id"])) == 2
    ...
    def test_test_windows_and_frame_labels_match_archive_intervals(self):
        ...
        assert concept.features.shape == (4, 2048)
        assert len(concept.windows) == 4
        assert int(labels["Abuse/Abuse002_x264.mp4"].sum()) == 16
        assert int(labels["Normal_Videos_event/Normal003_x264.mp4"].sum()) == 0
        anomaly_windows = [window for window in concept.windows if window.video_id == "Abuse/Abuse002_x264.mp4"]
        assert [window.label for window in anomaly_windows] == [0, 1]
